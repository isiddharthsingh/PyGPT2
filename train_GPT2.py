...
# Suppress warnings related to MPS autocast dtype and other UserWarnings
warnings.filterwarnings("ignore", message=".*MPS Autocast only supports dtype of torch.bfloat16 currently.*")
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # Flash Attention implementation we use instead
...
class GPTConfig:
    block_size: int = 1024
    vocab_size: int = 50304  # Correct vocab size
    n_layer: int = 12
    n_head: int = 12
...
if torch.cuda.is_available():
    torch.cuda.manual_seed(1337)  # Set seed for CUDA
else:
    print("Using device: mps or cpu")

total_batch_size = 524288 # 2**19, ~ 0.5M tokens
B = 16 # batch size
T = 256 # context size
assert total_batch_size % (B * T * ddp_world_size) == 0, "total batch size must be divisible by B * T * ddp_world_size"
grad_accum_steps = total_batch_size // (B * T *ddp_world_size)  # accumulate gradients over this many steps
if master_process:    
...
model = torch.compile(model, backend="aot_eager")  # Compile the model
if ddp:
    model = DDP(model, device_ids= [ddp_local_rank])
raw_model = model.module if ddp else model
#logits,loss = model(x,y)
#print(logits.shape)  # (B, T, vocab_size)
#print(loss)  #tensor(10.9455, device='mps:0', grad_fn=<NllLossBackward0>)
...
def get_lr(it):
    # kinear warmup for warmup_iters steps
    if it < warmup_steps:
        return max_lr * (it+1) / warmup_steps
    # if it > le_decay_iyters, return min learining rate
    if it > max_steps:
        return min_lr
...
for step in range(max_steps):
    ...
    if master_process:
        print(f"step {step}, loss: {loss.item()},learning rate: {lr:.2e}, loss_accum: {loss_accum.item()},norms: {norm:.2f} dt: {dt:.2f}ms, tok/s: {tokens_per_sec:.2f}")
...
# Now let's continue with the rest of your code
...
# prefix tokens
enc = tiktoken.get_encoding('gpt2')
tokens = enc.encode("Helllo, I'm a language model, ")
tokens = torch.tensor(tokens, dtype=torch.long).to(device)  # Ensure tokens are moved to device
tokens = tokens.unsqueeze(0).repeat(num_return_sequences, 1)  # (5,8)
x = tokens.to(device)  # Move input to the device

# generate! right now the x is (B,T) where B=5 and T=8
torch.manual_seed(42)
torch.cuda.manual_seed(42)  # You can still keep this in case you later switch to CUDA
